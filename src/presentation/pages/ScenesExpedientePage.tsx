  ...
  return (
    <Box>
      <Box display="flex" justifyContent="space-between" mb={3} flexWrap="wrap" gap={2}>
        ...
        <Button variant="contained" onClick={()=>navigate(`/dashboard/expedientes/${id}/escenas/new`)}>Nueva Escena</Button>
      </Box>
      <Card sx={{ p:0 }}>
        <Box overflow="auto">
              ...
              {loading ? <tr><td colSpan={7} style={{ padding:20 }}>Cargando...</td></tr> :
               ...
               items.map(s=>(
                <tr key={s.id_escena} style={{ borderBottom:'1px solid #e0e0e0' }}>
                  ...
                  <td style={{ padding:10 }}>
                    <Button size="small" variant="outlined" onClick={()=>navigate(`/dashboard/expedientes/${id}/escenas/${s.id_escena}/edit`)}>Editar</Button>{' '}
                    <Button size="small" variant="text" onClick={()=>navigate(`/dashboard/expedientes/${id}/escenas/${s.id_escena}/indicios`)}>Indicios</Button>{' '}
