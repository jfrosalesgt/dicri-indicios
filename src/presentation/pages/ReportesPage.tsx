...
export const ReportesPage = () => {
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        setStatsError(res.message || 'Error');
      }
    } catch(e: any) { 
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      const res = await reportesRepository.getRevisionExpedientes({
        ...
        estado_revision: data.estado || undefined
      });
      if (res.success && res.data) {
        setItems(res.data);
      } else {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(r => {
      const codigo = (r.codigo_caso ?? '').toLowerCase();
      const nombre = (r.nombre_caso ?? '').toLowerCase();
      const fiscalia = (r.nombre_fiscalia ?? '').toLowerCase();
      const tecnico = (r.tecnico_registra ?? '').toLowerCase();
      const coordinador = (r.coordinador_revision ?? '').toLowerCase();
      const justificacion = (r.justificacion_revision ?? '').toLowerCase();
      
      return codigo.includes(q) || nombre.includes(q) || fiscalia.includes(q) || 
             tecnico.includes(q) || coordinador.includes(q) || justificacion.includes(q);
    });
    ...
    <Box>
      ...
      <Card sx={{ p: 3, mb: 4, borderRadius: 4 }}>
        ...
        {!loading && !error && items.length === 0 && (
          <Card sx={{ p: 3 }}>
            <Typography fontStyle="italic">Sin datos para el rango seleccionado</Typography>
          </Card>
        )}
        
        {!loading && !error && filteredItems.length === 0 && items.length > 0 && (
          <Alert severity="info" sx={{ mb: 2 }}>
            Sin coincidencias con el filtro de texto.
          </Alert>
        )}
